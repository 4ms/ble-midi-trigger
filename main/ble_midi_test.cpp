...
static xQueueHandle trig_queue;
...
static void IRAM_ATTR trig_isr_handler(void* arg)
{
	...
	//printf("trig_isr_handler added to trig_queue gpio_num=%d", (uint32_t)gpio_num);
	//ESP_LOGI("TEST", "trig_isr_handler");
...
void app_main(void) {
	...
	//adc_setup();
	trig_queue = xQueueCreate(10, sizeof(uint32_t));
	start_pin_read(GPIO_NUM_4, GPIO_INTR_ANYEDGE, (gpio_isr_t)trig_isr_handler);
